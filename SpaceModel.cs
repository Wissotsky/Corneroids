...
namespace CornerSpace
{
  public class SpaceModel : IDisposable
  {
    ...
    public void Render(IRenderCamera camera)
    {
      ...
      Engine.GraphicsDevice.SetVertexBuffer(this.modelVB);
    ...
    private void CreateSpaceModel(int starCount)
    {
      ...
      vertexPositionColorList.AddRange(vertices);
      shortList.AddRange(indices);
      for (int index = 0; index < starCount; ++index)
      {
        Vector3 vector2 = ChooseRandomSpherePoint(rng);
        Matrix identity = Matrix.Identity;
        Matrix matrix1 = !(vector2 == Vector3.Up) ? (!(vector2 == Vector3.Down) ? Matrix.CreateFromAxisAngle(Vector3.Normalize(Vector3.Cross(-Vector3.UnitZ, vector2)), (float)Math.Acos(Vector3.Dot(-Vector3.UnitZ, vector2))) : Matrix.CreateRotationZ(4.712389f)) : Matrix.CreateRotationZ(1.57079637f);
      ...
      try
      {
        this.modelVB = new VertexBuffer(Engine.GraphicsDevice, VertexPositionColor.VertexDeclaration, vertexPositionColorList.Count, BufferUsage.WriteOnly);
        this.modelIB = new IndexBuffer(Engine.GraphicsDevice, IndexElementSize.SixteenBits, shortList.Count, BufferUsage.WriteOnly);
        this.modelVB.SetData(vertexPositionColorList.ToArray());
        this.modelIB.SetData(shortList.ToArray());
